-- =========================================================================
-- ระบบจัดการการเงินนักศึกษา โครงการครูคืนถิ่น — Supabase Schema (Option A)
-- ระบบ login ด้วย username/password ของตัวเอง เก็บรหัสผ่านแบบ hash (bcrypt ผ่าน pgcrypto)
-- ไม่ใช้ Supabase Auth / ไม่ต้องมี Edge Function ทุกอย่างทำงานผ่าน RPC function ในไฟล์นี้
--
...
create index idx_profiles_advisor_id on public.profiles (advisor_id);
create index idx_profiles_role on public.profiles (role);

-- เซสชันการเข้าสู่ระบบ (แทนที่ Supabase Auth session)
...
create table public.accounts (
  ...
  name text not null,
  type text not null check (type in ('cash','spending','holding','savings')),
  created_at timestamptz not null default now()
);
create index idx_accounts_student on public.accounts (student_id);
...
create index idx_expcats_student on public.expense_categories (student_id);
...
create index idx_inccats_student on public.income_categories (student_id);

-- หมายเหตุ: category_id / account_id / from_account_id / to_account_id ไม่ผูก FK ตรงๆ
-- เพราะ category_id อ้างอิงได้ทั้ง expense_categories หรือ income_categories แล้วแต่ type
-- ฝั่ง RPC เป็นผู้ตรวจสอบความถูกต้องตอนบันทึกแทน
...
create index idx_txn_student_date on public.transactions (student_id, date);

-- =========================================================================
-- ล็อกตารางทั้งหมด: เปิด RLS แต่ "ไม่สร้าง policy ใดๆ" เพื่อปิดกั้น REST API
-- โดยตรงทั้งหมด (deny-by-default) — ข้อมูลทั้งหมดต้องผ่าน RPC function ด้านล่างเท่านั้น
-- (ฟังก์ชันที่สร้างจาก SQL Editor เป็นของ role "postgres" ซึ่งมีสิทธิ์ bypass RLS อยู่แล้ว)
-- =========================================================================
...
revoke all on public.profiles, public.sessions, public.accounts,
  public.expense_categories, public.income_categories, public.transactions
